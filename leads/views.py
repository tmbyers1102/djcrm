...
class LeadListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_organizer:
            ...
            # once QS is defined down to one org, need to narrow down to specific agent thats logged in
    ...
    def get_context_data(self, **kwargs):
        ...
        user = self.request.user
        if user.is_organizer:
            # narrows to one org by user's org as defined in profile model
            queryset = Lead.objects.filter(
                organization=user.userprofile,
                agent__isnull=True
                )
...
class LeadUpdateView(OrganizerAndLoginRequiredMixin, generic.UpdateView):
    ...
    def get_queryset(self):
        # grabs the logged in user (must be logged in)
        user = self.request.user
        # narrow all leads down to one Org
